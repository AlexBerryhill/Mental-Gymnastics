...
def get_socket() -> socket.socket:
    """
    Creates and returns a UDP socket bound to an available local port.
    :return: A UDP socket.
    """
    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    # Allow address reuse
    s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    # Bind to an available port (port 0 tells the OS to choose one)
    s.bind(('', 0))
    return s
...
def set_ap(ssid: str, password: str, address: tuple) -> None:
    ...
    # Set AP mode with the given SSID and password
...
def parse_args(args: list) -> argparse.Namespace:
    ...
    SSID and password default to 'Walking BCD' and 'Alexander', respectively.
    ...
    parser.add_argument('-s', '--ssid', help='SSID of the Wi‑Fi network', default='Walking BCD', required=False)
    parser.add_argument('-p', '--pwd', help='Password of the Wi‑Fi network', default='Alexander', required=False)
